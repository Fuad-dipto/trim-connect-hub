    ...
    <MobileShell>
      ...
      <div className="px-4 pt-3">
        <div className="grid grid-cols-2 rounded-xl bg-secondary p-1 text-sm font-medium">
          {(["upcoming", "past"] as const).map((tk) => (
            <button
              key={tk}
              onClick={() => setTab(tk)}
              className={cn(
                "h-9 rounded-lg capitalize transition",
                tab === tk ? "bg-card shadow-sm text-foreground" : "text-muted-foreground",
              )}
            >
              {t(tk)} ({tk === "upcoming" ? upcoming.length : past.length})
            </button>
          ))}
        </div>
      </div>
      <div className="px-4 py-4 space-y-3">
        {(tab === "upcoming" ? upcoming : past).map((b) => (
          <BCard
            ...
            booking={b}
            isPast={tab === "past"}
            onCancel={() => { setCancelled((c) => [...c, b.id]); toast.success(t("Booking cancelled. Refund issued.")); }}
            t={t}
          />
        ))}
        {tab === "upcoming" && upcoming.length === 0 && (
          <div className="rounded-2xl border border-dashed border-border p-8 text-center">
            ...
            <Link to="/home"><Button className="mt-3 liquid-glass">{t("Book a salon")}</Button></Link>
          </div>
        )}
      </div>
    </MobileShell>
...
function BCard({ booking, isPast, onCancel, t }: { booking: Booking; isPast: boolean; onCancel: () => void; t: (s: string) => string }) {
      ...
      {!isPast ? (
        <div className="grid grid-cols-5 gap-1.5 pt-1 border-t border-border">
          <Link to="/booking-success" search={receiptSearch} className="flex flex-col items-center gap-0.5 py-2 rounded-lg hover:bg-secondary/60 text-[10px] font-medium">
            <Receipt className="h-4 w-4 text-primary"/>{t("Receipt")}
          </Link>
          <Link to="/chat/$barberId" params={{ barberId: barber.id }} className="flex flex-col items-center gap-0.5 py-2 rounded-lg hover:bg-secondary/60 text-[10px] font-medium">
            <MessageCircle className="h-4 w-4 text-primary"/>{t("Chat")}
          </Link>
          <button
            onClick={() => toast.info(t("Reschedule flow opening soon."))}
            className="flex flex-col items-center gap-0.5 py-2 rounded-lg hover:bg-secondary/60 text-[10px] font-medium"
          >
            <CalendarClock className="h-4 w-4 text-primary"/>{t("Reschedule")}
          </button>
          <a
            ...
            className="flex flex-col items-center gap-0.5 py-2 rounded-lg hover:bg-secondary/60 text-[10px] font-medium"
          >
            <Navigation className="h-4 w-4 text-primary"/>{t("Directions")}
          </a>
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>{t("Cancel booking")} {id}?</AlertDialogTitle>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction onClick={onCancel} className="bg-destructive hover:bg-destructive/90">{t("Cancel booking")}</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </div>
        ...
        <div className="grid grid-cols-3 gap-1.5 pt-1 border-t border-border">
          ...
          <button onClick={() => toast.success(t("Thanks for rating!"))} className="flex items-center justify-center gap-1 py-2 rounded-lg hover:bg-secondary/60 text-xs font-medium">
            <Star className="h-4 w-4 text-accent"/>{t("Rate")}
          </button>
          <Link to="/book/$barberId" params={{ barberId: barber.id }} className="flex items-center justify-center gap-1 py-2 rounded-lg bg-primary text-primary-foreground text-xs font-semibold">
            <RotateCcw className="h-3.5 w-3.5"/>{t("Book again")}
          </Link>
        </div>
      )}
    </div>
